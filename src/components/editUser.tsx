...
import { GetDetailUser, PostDetailUser } from '../apidata/API';
...
const EditUser = () => {
    ...
    const handleSubmit = async (e: { preventDefault: () => void; }) => {
        ...
        if (hasTrue) {
            try {
                const { name, username, email, company } = formData;
                await PostDetailUser({name, username, email, company },id);
                setFormData({
                    name: '',
                    username: '',
                    email: '',
    ...
    return (
        <div className="container d-flex justify-content-center align-items-center min-vh-100">
            <div className="card shadow-lg p-4" style={{ maxWidth: '400px', width: '100%' }}>
                <h2 className="text-center text-primary mb-4">Edit Data Here</h2>
